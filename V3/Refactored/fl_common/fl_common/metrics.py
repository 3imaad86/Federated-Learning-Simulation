...
GLOBAL_HEADER = [
    ...
    "n_train_replied", "n_train_aggregated",
...
PARTICIPATION_HEADER = ["client_id", "times_replied", "times_aggregated"]
...
def log_round(server_round, global_accuracy, global_loss, comm_cost_mb,
              ...
              n_train_replied=0, n_train_aggregated=0,
    ...
    _append(
        ...
        [server_round, float(global_accuracy), float(global_loss), float(comm_cost_mb),
         ...
         int(n_train_replied), int(n_train_aggregated),
...
def log_summary(total_time_s, accs_history, participation_counts,
    ...
    if (
        isinstance(participation_counts, dict)
        and "aggregated" in participation_counts
        and isinstance(participation_counts["aggregated"], dict)
    ):
        counts_source = participation_counts["aggregated"]
        if num_clients is not None:
            counts = [int(counts_source.get(cid, 0)) for cid in range(int(num_clients))]
        else:
            counts = list(counts_source.values())
    elif isinstance(participation_counts, dict):
        if num_clients is not None:
...
def log_participation(participation_counts, num_clients=None):
    ...
    if (
        isinstance(participation_counts, dict)
        and "replied" in participation_counts
        and "aggregated" in participation_counts
    ):
        replied = participation_counts["replied"]
        aggregated = participation_counts["aggregated"]
        client_ids = range(int(num_clients)) if num_clients is not None else sorted(
            set(replied) | set(aggregated)
        )
        rows = [
            [int(cid), int(replied.get(cid, 0)), int(aggregated.get(cid, 0))]
            for cid in client_ids
        ]
    elif num_clients is not None:
        rows = [
            [cid, int(participation_counts.get(cid, 0)), int(participation_counts.get(cid, 0))]
            for cid in range(int(num_clients))
        ]
    else:
        rows = [[int(cid), int(n), int(n)] for cid, n in sorted(participation_counts.items())]
