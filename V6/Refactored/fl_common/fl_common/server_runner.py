...
def run_federated_training(
    ...
    compr = f" comm-size-ratio={comm_size_ratio}" if comm_size_ratio != 1.0 else ""
    sim = f" sim-model-mb={sim_model_mb}" if sim_model_mb > 0 else ""
    print(f"[config] algo={algo_name}{banner_extra} partitioning={partitioning}{extra} "
          f"num_rounds={num_rounds} lr={lr}{strag}{compr}{sim}")

    # Taille effective transmise par client apres compression (pour comm_cost_mb).
    base_mb = sim_model_mb if sim_model_mb > 0 else (
        model_size_bytes() / (1024.0 * 1024.0))
    model_mb = base_mb * comm_size_ratio
    device = get_device()
    ...
    def agg_train(records, wk):
        ...
        # Pour moyennes locales ponderees par num-examples (ignore les drops) :
        w_sum, loss_w_sum, acc_w_sum = 0, 0.0, 0.0
        ...
        for rec in recs:
            ...
            n_dropped += is_drop
            n_ex = int(mr.get("num-examples", 0))
            loc_loss = float(mr.get("local_eval_loss", 0.0))
            loc_acc = float(mr.get("local_eval_acc", 0.0))
            if not is_drop and n_ex > 0:
                w_sum += n_ex
                loss_w_sum += loc_loss * n_ex
                acc_w_sum += loc_acc * n_ex
            ...
            details.append({
                ...
                "local_loss": loc_loss, "local_acc": loc_acc,
            })
        info["train"] = {
            ...
            "local_loss": loss_w_sum / w_sum if w_sum > 0 else 0.0,
            "local_acc": acc_w_sum / w_sum if w_sum > 0 else 0.0,
        }
        return m
    ...
    energy_cumul = 0.0
    fl_time_total = 0.0   # somme des round_time_s, SANS eval serveur
    eval_time_total = 0.0 # temps d'eval serveur, juste pour info

    for r in range(1, num_rounds + 1):
        # 1) Un round FL : uniquement TRAIN cote clients
        #    On mesure STRICTEMENT la duree de strategy.start (pas l'eval).
        ...
        #    Ce temps n'est PAS compte dans round_time_s ni fl_time_total.
        ...
        gap_acc = tr["local_acc"] - ev["accuracy"]
        ...
        print(f"[round {r}] local : acc={tr['local_acc']:.3f} loss={tr['local_loss']:.3f} "
              f"(gap_acc={gap_acc:+.3f} = biais non-IID)")
    ...
    # total_time = temps FL pur (somme des strategy.start), SANS eval serveur.
    # wall_time  = temps total incluant eval + logs + copies (juste pour info).
    ...
    print(f"[done] total_time={total_time:.1f}s (eval_time={eval_time_total:.1f}s "
