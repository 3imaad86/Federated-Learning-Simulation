...
def read_common_config(context):
    ...
    return {
        ...
        # Override de la taille modele POUR LA SIMULATION uniquement (en MB).
        # 0.0 = utiliser la vraie taille (~0.09 MB ici, donc effet compression
        # invisible vs compute). Mettre p.ex. 10.0 pour simuler ResNet-18,
        # 100.0 pour BERT-base, et voir un vrai effet sur l'energie comm.
...
def decide_early_drop(straggler_sim, pid, round_idx, comm_size_ratio=1.0,
    ...
    - `sim_model_mb` : si > 0, override la taille du vrai modele (utile pour
      simuler un gros modele edge IoT realiste, p.ex. 10 MB).
    ...
    """
    base_mb = float(sim_model_mb) if sim_model_mb > 0 else (
        model_size_bytes() / (1024.0 * 1024.0))
    model_mb = base_mb * float(comm_size_ratio)
    if straggler_sim:
