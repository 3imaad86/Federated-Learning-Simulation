...
def train(msg: Message, context: Context):
    ...
    pid = c["pid"]
    # Reproducibilite : seed unique par client (et stable d'un round a l'autre)
    if c["seed"] >= 0:
        set_seed(c["seed"] + pid)
    ...
    # 3) Upload simule. Par defaut on prend `delay` (calcule meme sans
    #    straggler-sim) pour que l'energie reflete la taille transmise.
    #    Sleep + deadline seulement si straggler-sim=1.
