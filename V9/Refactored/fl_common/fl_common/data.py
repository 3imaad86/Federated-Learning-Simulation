...
def _apply_data_hetero(tr_idx, pid, num_partitions):
    """Tronque le train du client a une fraction (0.2..1.0) qui depend du pid."""
...
def load_data(pid, num_partitions, batch_size, data_hetero=0,
              partitioning="noniid", alpha=0.3, seed=SEED):
    ...
    """
    ds = get_trainset()
    idx = np.array(get_partitions(num_partitions, partitioning, alpha, seed)[pid])
    np.random.default_rng(int(seed) + pid).shuffle(idx)
