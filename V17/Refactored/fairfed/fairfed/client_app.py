...
Specifite : avant l'entrainement local, le client calcule F_k = loss du
modele GLOBAL sur sa partition LOCALE. Cette valeur est envoyee via le
metric "f_k" pour que le serveur puisse calculer la deviation par rapport
au consensus et reponderer l'agregation.
"""
...
def train(msg: Message, context: Context):
    ...
    # 3) IMPORTANT FairFed : F_k = loss du modele GLOBAL sur partition locale
    #    AVANT training (signal de "satisfaction" du client).
    ...
    # 6) Eval locale (gap non-IID) + envoi de f_k
