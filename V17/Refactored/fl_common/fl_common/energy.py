...
def battery_for_tier(base_battery_j, tier):
    """Retourne la capacite batterie effective selon le tier compute.

    base_battery_j <= 0 garde le mode unlimited. Sinon, `base_battery_j`
    represente la batterie d'un client medium; weak/strong sont scales par
    BATTERY_TIER_MULTIPLIER.
    """
    if base_battery_j <= 0:
        return 0.0
