...
def print_banner(algo_name, cfg, banner_extra=""):
    ...
    print(f"[config] algo={algo_name}{banner_extra} partitioning={cfg['partitioning']}"
          f"{extra} num_rounds={cfg['num_rounds']} lr={cfg['lr']}{mom}{strag}{compr}{sim}{sd}")
...
def run_federated_training(
    ...
    Le cout de communication est mesure REELLEMENT a partir des bytes transmis
    dans les messages (par FedAvgDropFilter). Pas besoin de facteur a la main :
    SCAFFOLD verra automatiquement ~2x FedAvg parce qu'il envoie c_global / delta_c
    en plus des poids.
    """
    ...
    for r in range(1, cfg["num_rounds"] + 1):
        ...
        # Cout reel des messages (downlink + uplink) mesure par la strategy.
        # Le ratio comm-size-ratio simule la compression/quantification.
        real_bytes = strategy.last_downlink_bytes + strategy.last_uplink_bytes
        comm_mb = (real_bytes / (1024.0 * 1024.0)) * cfg["comm_size_ratio"]
        ...
        # Si tous les clients selectionnes ont droppe (batterie vide, deadline,
        # panne reseau), on conserve les derniers poids globaux valides et on
        # arrete proprement au lieu de continuer des rounds sans contribution.
        if tr["n_clients"] > 0 and tr["n_active"] == 0:
            print(f"[early-stop] aucun client actif a r={r}; "
                  "conservation du dernier modele global valide.")
            break
