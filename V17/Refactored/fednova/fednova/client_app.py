...
def train(msg: Message, context: Context):
    ...
    # Garde-fou FedNova : la formule tau_eff (Option C) suppose vanilla SGD.
    # Avec momentum > 0 le buffer momentum amplifie les Δ_i et la formule
    # devient une approximation. On previent UNE fois (pid=0, round=1).
    if pid == 0 and round_idx == 1 and c["momentum"] > 0:
        print(f"[fednova] WARN: momentum={c['momentum']} > 0, formule tau_eff "
              f"devient approximative (theorie suppose vanilla SGD).")
    ...
    trainloader, _ = load_data(pid, c["num_parts"], c["bs"],
                               ...
                               alpha=c["dir_alpha"],
                               seed=c["seed"])
    # NB : FedNova suppose vanilla SGD (momentum=0) dans la theorie. Avec
    # momentum > 0, la formule tau_i devient une approximation. Mais comparer
    # FedAvg-M vs FedNova-M (les 2 avec momentum) reste valide en pratique :
    # le ratio relatif d'agregation entre clients est preserve.
    # Par defaut on prend le momentum de la config (comme fedavg).
